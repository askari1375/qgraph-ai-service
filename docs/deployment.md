...
| `QGRAPH_AI_SEGMENTATION_ARTIFACTS_DIR` | Local prepared segmentation artifact directory |
...
startup. For production imports, copy reviewed artifact files into the image or
mount a host directory at that path before Django calls the artifact endpoints.
