...
This table lists only variables the **service** reads. Orchestration values — the
host port mapping, the shared Docker network, and host mount paths such as the
segmentation-artifacts host directory — are owned by the central `qgraph-deploy`
stack (its `.env` and `compose/production/env/ai-service.env`), not by this
service's env.
...
`QGRAPH_AI_SEGMENTATION_ARTIFACTS_DIR` (the in-container path, set in the service
env) and are not generated by the service at startup. The central `qgraph-deploy`
stack mounts a host directory read-only at that path; the host directory is set
via `QGRAPH_AI_SEGMENTATION_ARTIFACTS_HOST_DIR` in **qgraph-deploy's stack `.env`**
(not in the service env):

```env
# service env (qgraph-deploy/compose/production/env/ai-service.env)
QGRAPH_AI_SEGMENTATION_ARTIFACTS_DIR=/app/data/segmentation_artifacts
# stack env (qgraph-deploy/.env)
