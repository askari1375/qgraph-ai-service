...
Production deployment is intended to be private:

- Public clients call the Django backend.
- Django and Celery call this FastAPI service over the shared Docker network at
  `http://ai-backend:8001`.
- The VPS host keeps `127.0.0.1:8001` only for local smoke checks.
